 ...
 * @method SettingQuery orderByKeyName($order = Criteria::ASC) Order by the key_name column
 * @method SettingQuery orderByKeyValue($order = Criteria::ASC) Order by the key_value column
 *
 * @method SettingQuery groupById() Group by the id column
 * @method SettingQuery groupByKeyName() Group by the key_name column
 * @method SettingQuery groupByKeyValue() Group by the key_value column
 *
 ...
 *
 * @method Setting findOneByKeyName(string $key_name) Return the first Setting filtered by the key_name column
 * @method Setting findOneByKeyValue(string $key_value) Return the first Setting filtered by the key_value column
 *
 * @method array findById(string $id) Return Setting objects filtered by the id column
 * @method array findByKeyName(string $key_name) Return Setting objects filtered by the key_name column
 * @method array findByKeyValue(string $key_value) Return Setting objects filtered by the key_value column
 *
...
abstract class BaseSettingQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'geobin', $modelName = 'Setting', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT `id`, `key_name`, `key_value` FROM `setting` WHERE `id` = :p0';
        try {
    ...
    public function filterByKeyName($keyName = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(SettingPeer::KEY_NAME, $keyName, $comparison);
    }
     ...
     * $query->filterByKeyValue('%fooValue%'); // WHERE key_value LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByKeyValue($keyValue = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($keyValue)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $keyValue)) {
                $keyValue = str_replace('*', '%', $keyValue);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(SettingPeer::KEY_VALUE, $keyValue, $comparison);
    }
